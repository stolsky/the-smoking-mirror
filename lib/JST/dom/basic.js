 ...
 * @returns {boolean}
 */
const isUnique = (id) => isString(id) && document.getElementById(id) !== null;
 ...
 */
const setAttributesFromOptions = (target, options, allowedAttributes) => {
    if (target instanceof HTMLElement && options instanceof Object && allowedAttributes instanceof Array) {

        // TODO Object.values(options) || Object.keys(options) forEach(..)
...
const create = (tagName, className = null) => {
    ...
    if (isString(tagName)) {
        ...
        obj = document.createElement(tagName.toLowerCase());
...
const insertStyleSheet = (path) => document.styleSheets[0].insertRule(`@import url(${path})`, 0);
